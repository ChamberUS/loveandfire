...
import { isUserSessionValid, getUserSession } from '@/auth/userAuth';
...
export default function AuthLogin() {
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await loginUser({ email: normalizedEmail, password, rememberMe });
      refreshSessions();
    ...
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden dark bg-[#070B0F] text-white">
      ...
      <div className="w-full max-w-md relative z-10">
        <GlassCard className="p-8">
          ...
          <p className="text-white/50 mb-6">Acesse sua conta no AIOS.</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <p className="text-center text-white/40 text-sm">
              Não tem conta?{' '}
              <Link to="/auth/register" className="text-emerald-400 hover:text-emerald-300">
                Criar conta
              </Link>
            </p>
          </form>
